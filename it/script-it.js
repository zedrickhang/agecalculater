...
来源：精确年龄计算器`;
    ...
    saveResult(ageData) {
        ...
        ctx.fillText('🎂 Risultato Calcolo Età', canvas.width / 2, 80);
        
        // Risultato principale
        ctx.font = 'bold 48px Arial, sans-serif';
        const yearText = years === 1 ? 'Anno' : 'Anni';
        const monthText = months === 1 ? 'Mese' : 'Mesi';
        const dayText = days === 1 ? 'Giorno' : 'Giorni';
        ctx.fillText(`${years} ${yearText} ${months} ${monthText} ${days} ${dayText}`, canvas.width / 2, 160);
        
        // Titolo statistiche dettagliate
        ctx.font = 'bold 24px Arial, sans-serif';
        ctx.fillText('📊 Statistiche Dettagliate', canvas.width / 2, 220);
        
        // Contenuto statistiche dettagliate
        ctx.font = '20px Arial, sans-serif';
        const stats = [
            `📅 ${ageData.totalMonths.toLocaleString()} mesi ${days} giorni`,
            `📊 ${ageData.totalWeeks.toLocaleString()} settimane ${ageData.totalDays % 7} giorni`,
            `🌅 ${ageData.totalDays.toLocaleString()} giorni`,
            `⏰ ${ageData.totalHours.toLocaleString()} ore`,
            `⏱️ ${ageData.totalMinutes.toLocaleString()} minuti`,
            `⚡ ${ageData.totalSeconds.toLocaleString()} secondi`
        ];
        ...
        ctx.fillText(`Calcolato il: ${new Date().toLocaleString('it-IT')}`, canvas.width / 2, 520);
        ctx.fillText('Fonte: Calcolatore di Età Preciso - agecomputation.com', canvas.width / 2, 550);
        ...
        canvas.toBlob((blob) => {
            ...
            link.href = url;
            link.download = `risultato_calcolo_eta_${new Date().toISOString().split('T')[0]}.png`;
            link.click();
            
            URL.revokeObjectURL(url);
            
            // Mostra messaggio di successo
            this.showSuccess('Immagine risultato calcolo età salvata!');
        }, 'image/png');
    }
