...
来源：精确年龄计算器`;
    ...
    saveResult(ageData) {
        ...
        ctx.fillText('🎂 年龄计算结果', canvas.width / 2, 80);
        
        // 主要结果
        ctx.font = 'bold 48px Arial, sans-serif';
        ctx.fillText(`${years}年 ${months}月 ${days}天`, canvas.width / 2, 160);
        
        // 详细统计标题
        ctx.font = 'bold 24px Arial, sans-serif';
        ctx.fillText('📊 详细统计', canvas.width / 2, 220);
        ...
        const stats = [
            ...
            `🌅 ${ageData.totalDays.toLocaleString()} 天`,
            `⏰ ${ageData.totalHours.toLocaleString()} 小时`,
            `⏱️ ${ageData.totalMinutes.toLocaleString()} 分钟`,
            `⚡ ${ageData.totalSeconds.toLocaleString()} 秒`
        ];
        ...
        ctx.fillText(`计算时间: ${new Date().toLocaleString('zh-CN')}`, canvas.width / 2, 520);
        ctx.fillText('来源: 精确年龄计算器 - agecomputation.com', canvas.width / 2, 550);
        ...
        canvas.toBlob((blob) => {
            ...
            link.href = url;
            link.download = `年龄计算结果_${new Date().toISOString().split('T')[0]}.png`;
            link.click();
            
            URL.revokeObjectURL(url);
            
            // 显示成功提示
            this.showSuccess('年龄计算结果图片已保存！');
        }, 'image/png');
    }
