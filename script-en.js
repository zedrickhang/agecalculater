...
class AgeCalculator {
    ...
    calculateAge() {
        ...
        if (birthDate > targetDate) {
            this.showError('Birth date cannot be later than target date');
            return;
        }
    ...
    generateSummaryText(ageData) {
        ...
        return summary ? `You have lived for ${summary}` : 'Calculation completed';
    }
    ...
    generateDetailedStats(ageData) {
        const stats = [
            {
                label: 'or',
                value: `${ageData.totalMonths.toLocaleString()} months ${ageData.days} days`,
                icon: '📅'
            },
            {
                label: 'or',
                value: `${ageData.totalWeeks.toLocaleString()} weeks ${ageData.totalDays % 7} days`,
                icon: '📊'
            },
            {
                label: 'or',
                value: `${ageData.totalDays.toLocaleString()} days`,
                icon: '🌅'
            },
            {
                label: 'or',
                value: `${ageData.totalHours.toLocaleString()} hours`,
                icon: '⏰'
            },
            {
                label: 'or',
                value: `${ageData.totalMinutes.toLocaleString()} minutes`,
                icon: '⏱️'
            },
            {
                label: 'or',
                value: `${ageData.totalSeconds.toLocaleString()} seconds`,
                icon: '⚡'
            }
        ];
    ...
    shareResult(ageData) {
        const { years, months, days } = ageData;
        const text = `🎂 Age Calculation Result: ${years} years ${months} months ${days} days
        
📊 Detailed Statistics:
• ${ageData.totalMonths.toLocaleString()} months ${days} days
• ${ageData.totalWeeks.toLocaleString()} weeks ${ageData.totalDays % 7} days  
• ${ageData.totalDays.toLocaleString()} days
• ${ageData.totalHours.toLocaleString()} hours
• ${ageData.totalMinutes.toLocaleString()} minutes
• ${ageData.totalSeconds.toLocaleString()} seconds

Source: Precise Age Calculator`;
    ...
    saveResult(ageData) {
        ...
        ctx.fillText('🎂 Age Calculation Result', canvas.width / 2, 80);
        ...
        ctx.fillText('📊 Detailed Statistics', canvas.width / 2, 220);
        ...
        const stats = [
            ...
            `🌅 ${ageData.totalDays.toLocaleString()} days`,
            `⏰ ${ageData.totalHours.toLocaleString()} hours`,
            `⏱️ ${ageData.totalMinutes.toLocaleString()} minutes`,
            `⚡ ${ageData.totalSeconds.toLocaleString()} seconds`
        ];
        ...
        ctx.fillText(`Calculated on: ${new Date().toLocaleString('en-US')}`, canvas.width / 2, 520);
        ctx.fillText('Source: Precise Age Calculator - agecomputation.com', canvas.width / 2, 550);
        ...
        canvas.toBlob((blob) => {
            ...
            link.href = url;
            link.download = `age_calculation_result_${new Date().toISOString().split('T')[0]}.png`;
            link.click();
            
            URL.revokeObjectURL(url);
            
            // 显示成功提示
            this.showSuccess('Age calculation result image saved!');
        }, 'image/png');
    }
    ...
    fallbackCopyTextToClipboard(text) {
        ...
        try {
            ...
            this.showError('Copy failed, please copy the result manually');
        }
